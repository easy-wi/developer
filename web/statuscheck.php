...
if (!isset($ip) or $_SERVER['SERVER_ADDR']==$ip) {
    ...
    $query=$insert=$sql->prepare("UPDATE `lendsettings` SET `oldcheck`=`lastcheck`,`lastcheck`=NOW()");
    $query->execute();
    ...
    if ($checkTypeOfServer=='all' or $checkTypeOfServer=='gs') {
        ...
        foreach ($other as $array) {
            ...
            foreach ($query->fetchall(PDO::FETCH_ASSOC) as $row) {
                ...
                if ($lendserver=='Y' and $resellersettings[$resellerid]['active']=='Y' and $resellersettings[$resellerid]['shutdownempty']=='Y') {
                    $shutdownemptytime=$resellersettings[$resellerid]['shutdownemptytime'];
                    $elapsedtime=$sql->prepare("SELECT `id`,`started` FROM `lendedserver` WHERE `serverid`=? LIMIT 1");
                    $elapsedtime->execute(array($row['id']));
            ...
            if (isset($userid) and isset($switchID)) {
                $status="UP";
                if ($qstat=='gtasamp') {
                    $query=GTASAMP($serverip,$port);
                    if (is_array($query)) {
                        $name=$query['hostname'];
                ...
                } else if ($qstat=='minecraft') {
                    $query=MineCraft($serverip,$port);
                    if (is_array($query)) {
                        $name=$query['hostname'];
                        $password="Y";
                        $numplayers=$query['players'];
        ...
        foreach ($vselect->fetchall(PDO::FETCH_ASSOC) as $vrow) {
            ...
            if ($vrow['type']=='ts3') {
                ...
                if ($tsdown==true or $tsdnsdown==true) {
                    ...
                    $pupdate=$sql->prepare("UPDATE `voice_masterserver` SET `notified`=? WHERE `id`=? LIMIT 1");
                ...
                if ($tsdown!=true) {
                    ...
                    if (!isset($serverlist['0']['id']) or $serverlist['0']['id']=='0') {
                        foreach ($serverlist as $server) {
                            ...
                            if (isset($ts3id)) {
                                $flagPassword='N';
                                if(isset($sd['virtualserver_flag_password']) and $sd['virtualserver_flag_password']==1) $flagPassword='Y';
                                $update=$sql->prepare("UPDATE `voice_server` SET `usedslots`=?,`uptime`=?,`notified`=?,`filetraffic`=?,`lastfiletraffic`=?,`queryName`=?,`queryNumplayers`=?,`queryMaxplayers`=?,`queryPassword`=?,`queryUpdatetime`=NOW() WHERE `id`=? AND `resellerid`=? LIMIT 1");
                                $update->execute(array($usedslots,$uptime,$newnotified,$newtraffic,$newtrafficdata,$queryName,((isset($server['virtualserver_clientsonline'])) ? $server['virtualserver_clientsonline'] : 0 - 1),(isset($server['virtualserver_maxclients'])) ? $server['virtualserver_maxclients'] : 0,$flagPassword,$ts3id,$resellerid));
                            }
                            if (isset($args['coolDown'])) {
                                $nano=time_nanosleep(0,$args['coolDown']);
                                if ($nano===true) echo 'Slept for '.$args['coolDown'].' microseconds'."\r\n";
