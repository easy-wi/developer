...
<div class="row-fluid span11 alert <?php if($table_row['stopped']=='N') echo 'alert-success'; else echo 'alert-block';?>">
    ...
    <div class="row-fluid">
        <div class="span12 btn-group-vertical">
            ...
            <a href="userpanel.php?w=gs&amp;d=cf&amp;id=<?php echo $table_row['id'];?>"><button class="btn btn-mini btn-info"><i class="icon-white icon-cog"></i> <?php echo $sprache->config;?></button></a>
            ...
            <a href="userpanel.php?w=ca&amp;id=<?php echo $table_row['id'];?>"><button class="btn btn-mini btn-info"><i class="icon-white icon-calendar"></i> <?php echo $sprache->restarttime;?></button></a>
            <a href="serverlog.php?id=<?php echo $table_row['id'];?>" onclick="return popup(this.href);"><button class="btn btn-mini btn-inverse"><i class="icon-white icon-tasks"></i> <?php echo $gsprache->logs;?></button></a>
            <a href="userpanel.php?w=bu&amp;id=<?php echo $table_row['id'];?>"><button class="btn btn-mini btn-info"><i class="icon-white icon-bold"></i> <?php echo $gsprache->backup;?></button></a>
            <?php if($pa['useraddons']){ ?><a href="userpanel.php?w=ao&amp;id=<?php echo $table_row['id'];?>"><button class="btn btn-mini btn-info"><i class="icon-white icon-gift"></i> <?php echo $gsprache->addon;?></button></a><?php } ?>
            <?php if(!empty($table_row['pro'])){ ?><a href="userpanel.php?w=pr&amp;id=<?php echo $table_row['id'];?>&amp;r=gs" onclick="return confirm('<?php echo $gsprache->sure;?>');"><button class="btn btn-mini <?php if($table_row['imgp']=='16_protected')echo 'btn-success';else if($table_row['imgp']=='16_unprotected') echo 'btn-danger';?>"><i class="icon-white icon-lock"></i> <?php echo $sprache->protect;?></button></a><?php } ?>
