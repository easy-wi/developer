...
if ($ui->w('action',4,'post') and !token(true)) {
    ...
    if (!$ui->smallletters('action',2,'post')) {
        $pselect=$sql->prepare("SELECT e.`dbname`,AES_DECRYPT(e.`password`,?) AS `decryptedpassword`,e.`ips`,s.`ip`,s.`port`,s.`interface`,u.`cname` FROM `mysql_external_dbs` e LEFT JOIN `mysql_external_servers` s ON e.`sid`=s.`id` LEFT JOIN `userdata` u ON e.`uid`=u.`id` WHERE e.`id`=? AND e.`active`='Y' AND s.`active` AND e.`resellerid`=? LIMIT 1");
        ...
        $fail=0;
        $error="";
        if (!is_password($ui->post['password'],40)) {
            $error .=" Password<br />";
            $fail="1";
        }
            ...
            foreach ($pselect->fetchall() as $row) {
                ...
                if ($old_password!=$password or $old_ips!=$ips) {
                    ...
                    if ($remotesql->error=='ok') {
                        $pupdate=$sql->prepare("UPDATE `mysql_external_dbs` SET `password`=AES_ENCRYPT(?,?),`ips`=? WHERE `id`=? AND `uid`=? AND `resellerid`=? LIMIT 1");
                        $pupdate->execute(array($password,$aeskey,$ips,$id,$user_id,$reseller_id));
