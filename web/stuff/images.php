...
if ($ui->w('action',4,'post') and !token(true)) {
    ...
    if ($ui->id('id',10,'get') and !$ui->smallletters('action',2,'post') and $ui->st('d','get')=='md') {
        $id=$ui->id('id',10,'get');
        $query=$sql->prepare("SELECT * FROM `servertypes` WHERE `id`=? AND `resellerid`=?");
        ...
        foreach ($query->fetchAll(PDO::FETCH_ASSOC) as $row) {
            ...
            $option=(isset ($qstat) and $qstat==$aqstat) ? "<option value=\"$aqstat\" selected=\"selected\">$adescription</option>" : "<option value=\"$aqstat\">$adescription</option>";
            $table3[]=array('option'=>$option);
        }
        $template_file=(isset($steamgame)) ? "admin_images_md.tpl" : 'admin_404.tpl';
    } else if ((!$ui->smallletters('action',2,'post') or $ui->id('import',1,'post')==1) and $ui->st('d','get')=='ad') {
        $token=token();
        $query=$sql->prepare("SELECT DISTINCT(`shorten`) FROM `servertypes` WHERE `resellerid`=?");
        $query->execute(array($reseller_id));
        $table = array();
        ...
        $query->execute();
        $table2 = array();
        foreach ($query->fetchAll(PDO::FETCH_ASSOC) as $row) $table2[]=array('qstat'=>$row['qstat'],'description'=>$row['description']);
        $steamgame='';
        $appID='';
        $updates='';
        $shorten='';
            ...
            try {
                ...
                if (@$xml->load($_FILES["file"]["tmp_name"])!==false) {
                    ...
                    foreach ($childNodes->childNodes AS $node) {
                        if($node->nodeName=='steamgame') $steamgame=$node->nodeValue;
                        if($node->nodeName=='appID') $appID=$node->nodeValue;
                        if($node->nodeName=='updates ') $updates=$node->nodeValue;
                        if($node->nodeName=='description') $description=$node->nodeValue;
                        if($node->nodeName=='type') $type=$node->nodeValue;
                        if($node->nodeName=='gamebinary') $gamebinary=$node->nodeValue;
                        if($node->nodeName=='binarydir') $binarydir=$node->nodeValue;
                        if($node->nodeName=='modfolder') $modfolder=$node->nodeValue;
                        if($node->nodeName=='fps') $fps=$node->configs;
                        if($node->nodeName=='slots') $slots=$node->nodeValue;
                        if($node->nodeName=='modcmds') $modcmds=$node->nodeValue;
                        if($node->nodeName=='tic') $tic=$node->nodeValue;
                        if($node->nodeName=='qstat') $qstat=$node->nodeValue;
                        if($node->nodeName=='gamemod') $gamemod=$node->nodeValue;
                        if($node->nodeName=='gamemod2') $gamemod2=$node->nodeValue;
                        if($node->nodeName=='configs') $configs=$node->nodeValue;
                        if($node->nodeName=='configedit') $configedit=$node->nodeValue;
                        if($node->nodeName=='qstatpassparam') $qstatpassparam=$node->nodeValue;
                        if($node->nodeName=='portStep') $portStep=$node->nodeValue;
                        if($node->nodeName=='portMax') $portMax=$node->nodeValue;
                        if($node->nodeName=='portOne') $portOne=$node->nodeValue;
                        if($node->nodeName=='portTwo') $portTwo=$node->nodeValue;
                        if($node->nodeName=='portThree') $portThree=$node->nodeValue;
                        if($node->nodeName=='portFour') $portFour=$node->nodeValue;
                        if($node->nodeName=='portFive') $portFive=$node->nodeValue;
                        if($node->nodeName=='protected') $protected=$node->nodeValue;
                        if($node->nodeName=='protectedSaveCFGs') $protectedSaveCFGs=$node->nodeValue;
                        if($node->nodeName=='iptables') $iptables=$node->nodeValue;
                        if($node->nodeName=='mapGroup') $mapGroup=$node->nodeValue;
                    }
                }
    ...
    } else if ($ui->smallletters('action',2,'post')=='ad' or ($ui->smallletters('action',2,'post')=='md' and $ui->id('id',10,'get'))){
        $error=array();
        if(!$ui->w('steamgame',1,'post')) $error[]='Steamgame';
        if(!$ui->active('gamemod','post')) $error[]='Gamemod';
        if(!$ui->id('updates',1,'post')) $error[]='Updates';
        if(in_array($ui->startparameter('cmd','post'),array('',null,false))) $error[]='No startcommand';
        if(!$ui->startparameter('gamebinary','post') or $ui->startparameter('gamebinary','post')=='') $error[]='Bad binaryname';
        ...
        if($ui->gamestring('shorten','post') and $ui->smallletters('action',2,'post')=='ad') {
            ...
            if($query->rowCount()>0) $error[]='Shorten';
            ...
            $query->execute(array($ui->id('id',10,'get'),$shorten,$reseller_id));
            if($query->rowCount()>0) $error[]='Shorten';
        } else {
            $error[]='Shorten';
        }
        if (count($error)>0){
            $template_file='Error: '.implode(', ',$error);
            ...
            if($query->rowCount()>0) {
                ...
                $template_file='Error could not write to table';
            }
        } else {
            $type='gserver';
            ...
            if($query->rowCount()>0) {
                ...
                $template_file='Error: could not update table or data unchanged!';
            }
        }
    } else {
        $template_file='admin_404.tpl';
    ...
    if (!isset($action)) {
        ...
        $template_file=(isset($description)) ? "admin_images_dl.tpl" : 'admin_404.tpl';
    } else if ($action=='dl'){
        ...
        if($query->rowCount()>0) {
            ...
            $template_file='Error could not edit table';
        }
    ...
    $table = array();
    foreach ($query->fetchAll(PDO::FETCH_ASSOC) as $row) $table[]=array('id'=>$row['id'],'shorten'=>$row['shorten'],'steamgame'=>$row['steamgame'],'type'=>$row['type'],'description'=>$row['description']);
    $next=$start+$amount;
