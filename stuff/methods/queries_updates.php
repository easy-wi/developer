...
function getMinecraftVersion($release = 'release') {
    ...
    $responseBody = webhostRequest('launchermeta.mojang.com', 'https://easy-wi.com', '/mc/game/version_manifest.json');

    $json = @json_decode(cleanFsockOpenRequest($responseBody, '{', '}'));
    $latest = $json->latest->$release;
    $spliturl = parse_url($json->versions[0]->url);

    $responseBody = webhostRequest($spliturl["host"], 'https://easy-wi.com', $spliturl["path"]);
    $json = @json_decode(cleanFsockOpenRequest($responseBody, '{', '}'));
