...
if (array_value_exists('action', 'ls', $data)) {
    ...
    if ($apiType == 'json' or $apiType == 'xml') {
        ...
        if (isset($data['type'])) {
            if (is_array($data['type']) and count($data['type']) > 0) {
                $listLimit = $data['type'];
            } else if (is_string($data['type'])) {
                $listLimit = array($data['type']);
            }
        }
        ...
        if ($apiType == 'xml') {
            ...
            $types = (isset($listLimit)) ? implode(',', $listLimit) : 'all';
            $key = $responsexml->createElement('types', $types);
        ...
        // TSDNS
        if (!isset($listLimit) or in_array('tsdnsServer', $listLimit)) {
        ...
        // Voice server
        if (!isset($listLimit) or in_array('voiceServer', $listLimit)) {
        ...
        // Webspace
        if (!isset($listLimit) or in_array('webspaceServer', $listLimit)) {
