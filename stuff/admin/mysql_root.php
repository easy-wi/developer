...
if ($ui->st('d', 'get') == 'bu' and $ui->st('action', 'post') == 'bu' and $reseller_id == 0) {
    ...
    if ($ui->st('action', 'post') == 're') {
        ...
        foreach ($templateRepair as $template) {
            ...
            if (isset($emailTemplates[$template])) {
                ...
                $query = $sql->prepare($emailTemplates[$template]['query']);
    ...
    if ($ui->st('action', 'post') == 'rg') {
        ...
        foreach ($gameImages as $image) {
            ...
            } else if (in_array($image[':shorten'], $array) and count($image) != 36) {
                $template_file .= 'Skipped: ' . $image[':description'] .'. Array size is ' . count($image) . ' instead of 36<br>';
            }
        }
